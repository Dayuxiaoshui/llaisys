...
void checkMcblas(mcblasStatus_t status, const char *operation) {
    if (status != MCBLAS_STATUS_SUCCESS) {
        throw std::runtime_error(std::string(operation) + " failed with mcBLAS status " + std::to_string(status));
    }
}

void checkLaunch(const char *kernel) {
    checkMaca(mcGetLastError(), kernel);
}

unsigned int gridSize(size_t numel) {
    const size_t blocks = (numel + BLOCK_SIZE - 1) / BLOCK_SIZE;
    CHECK_ARGUMENT(blocks <= std::numeric_limits<unsigned int>::max(), "MACA launch grid is too large.");
...
template <>
__device__ float toFloat<__maca_bfloat16>(__maca_bfloat16 value) {
...
template <>
__device__ __maca_bfloat16 fromFloat<__maca_bfloat16>(float value) {
...
void dispatchFloatTypes(llaisysDataType_t dtype, Function &&function) {
    ...
    case LLAISYS_DTYPE_BF16:
        return function(TypeTag<__maca_bfloat16>{});
    default:
...
public:
    ~McblasHandleCache() {
        if (_handle != nullptr) {
            mcblasDestroy(_handle);
        }
    }

    mcblasHandle_t get(mcStream_t stream) {
        int device = 0;
        checkMaca(mcGetDevice(&device), "mcGetDevice");
        if (_handle == nullptr || _device != device) {
            if (_handle != nullptr) {
                checkMcblas(mcblasDestroy(_handle), "mcblasDestroy");
            }
            checkMcblas(mcblasCreate(&_handle), "mcblasCreate");
            _device = device;
        }
        checkMcblas(mcblasSetStream(_handle, stream), "mcblasSetStream");
        return _handle;
    }
};

thread_local McblasHandleCache mcblas_cache;
} // namespace

void add(std::byte *out, const std::byte *a, const std::byte *b, llaisysDataType_t dtype, size_t numel, llaisysStream_t stream_) {
    const auto stream = reinterpret_cast<mcStream_t>(stream_);
...
void linear(std::byte *out,
    ...
    const auto stream = reinterpret_cast<mcStream_t>(stream_);
    const mcblasHandle_t handle = mcblas_cache.get(stream);
    const float alpha = 1.0f;
    const float beta = 0.0f;
    macaDataType data_type;
    // mcBLAS may lower COMPUTE_32F to TF32 on tensor-op paths; request the
    // pedantic compute type for FP32 so results match full-precision FP32.
...
void self_attention(std::byte *out,
    ...
    checkMaca(mcMallocAsync(reinterpret_cast<void **>(&scores), total_scores * sizeof(float), stream), "mcMallocAsync");
    ...
    checkMaca(mcFreeAsync(scores, stream), "mcFreeAsync");
}
